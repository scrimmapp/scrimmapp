...
export function ListingDetail({ id }: { id: string }) {
  ...
  if (!listing) {
    return (
      <div className="mx-auto max-w-lg px-4 py-24 text-center">
        <p className="font-display text-lg font-bold text-ink">Listing not found</p>
        <p className="mt-2 text-xs text-ink-2">It may have been cancelled or the link is out of date.</p>
        <Link href="/board">
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-2.5 px-4 py-3">
      <Link href="/board" className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-ink-2 transition-colors hover:text-pitch">
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        <Card className="space-y-3 p-3.5 lg:col-span-2 md:p-4">
          <div className="flex flex-wrap items-start justify-between gap-3 border-b border-rule pb-2.5">
            <div>
              ...
              <p className="mt-0.5 text-[10px] text-muted">Posted on the ScrimmApp board</p>
            </div>
            <button
              type="button"
              onClick={() => navigator.clipboard?.writeText(window.location.href)}
              className="rounded-control border border-rule-2 bg-paper px-2 py-1 text-[10px] font-bold text-ink-2 transition-colors hover:bg-surface-2"
            >
              Share link
            </button>
          </div>
          ...
          {listing.notes && (
            <div className="rounded-control border border-rule bg-paper p-2.5 text-[11px] text-ink-2">
              {listing.notes}
            </div>
          )}

          <div className="space-y-1 rounded-card border border-rule bg-paper p-2.5">
            <h4 className="text-[8px] font-bold uppercase tracking-wider text-muted">Venue & navigation</h4>
            <p className="text-[11px] font-bold text-ink">{listing.location}</p>
            <a
              href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(listing.location)}`}
              target="_blank"
              rel="noreferrer"
              className="inline-block rounded-control bg-pitch-bg px-2 py-1 text-[10px] font-bold text-pitch-ink transition-colors hover:brightness-95"
            >
        ...
        <Card className="flex flex-col justify-between gap-2.5 p-3.5">
          <div>
            <h3 className="mb-2 border-b border-rule pb-2 font-display text-sm font-bold text-ink">
                ...
                <AnimatePresence initial={false}>
                  {postComments.map((c) => (
                    <motion.div
                      ...
                      className="rounded-control border border-rule bg-paper p-2 text-[10px]"
                    >
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-control border border-rule bg-paper p-2">
      ...
      <span className={`text-[11px] font-bold ${accent ? "text-pitch" : "text-ink"}`}>{value}</span>
    </div>
  );
}
