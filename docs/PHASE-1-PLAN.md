...
| Schedule | 8 increments, Aug 12 – Oct 4 2026 |
| Go-live | Sun 4 Oct 2026, scrimmapp.com |
| Build cost | $3,000 – $3,750 at $20–25/h |
...
| Frontend | "fully built in React + Tailwind" | not restated | **Incorrect — §2** |
...
|---|---|---|---|
| Frontend + API | Next.js 15 (App Router), TypeScript, Tailwind v4 | One codebase for UI + API. SSR makes public listings indexable — real acquisition value. Mock's Tailwind classes port directly. | Vite SPA + separate API: two deploys, no SEO, more glue |
...
| Schema | Drizzle ORM + drizzle-kit | Type-safe queries; migrations committed as reviewable SQL — matters for handover. | Prisma: heavier runtime, awkward Cloud Run cold starts |
...
3. **Cancellations are their own rows.** Deleting a listing would destroy exactly the history Javi asked to capture. A cancelled listing keeps its row, changes status, gains a cancellation record — so repeat last-minute pull-outs become visible.
...
|---|---|---|---|
| **D1** | ✅ **Resolved 12 Aug.** Mock is a base reference only; screens are designed from the email requirements (§2). **No budget impact** — absorbed by AI-assisted velocity (§5). Worth a one-line note to Javi for the record, but not a blocker. | — | — |
| **D2** | ~~Supabase over Neon.~~ **Resolved 12 Aug:** Next.js + TypeScript + Tailwind confirmed by Bala. Supabase confirmed as Postgres + Auth + realtime (§12); Firebase Auth dropped — it cannot enforce DB-level access rules. Javi to countersign. | Aug 16 | S0 |
...
| **D8** | ✅ **Resolved 12 Aug.** Contingency comes from AI-assisted velocity, not from Javi's budget: 133 h estimated + 17 h buffer inside the agreed 150 (§5). Nothing to approve. | — | — |
...
| **Email lands in spam** | A brand-new domain sending transactional mail is untrusted by default. The Connect notification is the product's core loop — in junk, the platform silently fails. | DKIM/SPF/DMARC in S4, not at launch. Warm the domain gradually, watch SendGrid delivery stats through S5–S7. | Medium |
...
**Requirement: no running cost before production.** That is achievable, but not automatically — free tiers leak money quietly if nobody bounds them. The ledger and the guardrails below are what make $0 actually hold.
...
### Guardrails — without these, "$0" is a hope, not a plan
...
|---|---|---|
| **S0 – S6 · Aug 12 – Sep 27** | **$0** | Every layer inside a free tier, bounded by the guardrails above |
...
Supabase's free tier *can* technically run production for a tiny user base, but free projects pause after a week of inactivity — exactly wrong for a marketplace a coach checks monthly. **The $25 starts at S7 and not a sprint earlier.**
